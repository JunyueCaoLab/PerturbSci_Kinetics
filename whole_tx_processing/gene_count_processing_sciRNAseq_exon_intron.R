...
combine_exon_intron <- function (df_gene, gene_count) 
{
    ...
    if (nrow(gene_count_exon) == nrow(gene_count_intron)) {
        ...
        gene_count_intron = as(rbind(gene_count_intron, 0), "sparseMatrix")
    }
    return(list(gene_count_combine, gene_count_exon, gene_count_intron))
}
...
sciRNAseq_gene_count_summary <- function (gene_count_folder) {
    ...
    combine = combine_exon_intron(df_gene, gene_count)
    gene_count = combine[[1]]
    ...
    df_cell$UMI_count = df_cell_combine$all_exon + df_cell_combine$all_intron - df_cell_combine$X.ambiguous.match.for.exons - df_cell_combine$X.ambiguous.match.for.genes
    df_gene = df_gene %>% select(gene_id, gene_type, gene_name)
    return(list(df_cell, df_gene, gene_count, combine[[2]], combine[[3]]))
}
result = sciRNAseq_gene_count_summary(paste0(report_folder, "/human_mouse_gene_count/"))
df_cell = result[[1]]
df_gene = result[[2]]
gene_count_all = result[[3]]
gene_count_exon = result[[4]]
gene_count_intron = result[[5]]
save(df_cell, df_gene, gene_count_all, gene_count_exon, gene_count_intron, file = paste0(output_folder, "/Sci3_Summary.RData"))
